...
static void I2C_BusHardwareInitialize(struct I2C_BusTypeDef *bus)
{
    ...
    GPIO_InitStruct.GPIO_Pin = bus->scl_gpio_pin | bus->sda_gpio_pin;
...
static inline void SDA_SetOutMode(struct I2C_BusTypeDef *bus)
{
	...
	GPIO_InitStruct.GPIO_Pin = bus->sda_gpio_pin;
...
{
	return GPIO_ReadInputDataBit(bus->gpiox, bus->sda_gpio_pin);
}

static inline void SDA_L(struct I2C_BusTypeDef *bus)
{
    GPIO_WriteBit(bus->gpiox, bus->sda_gpio_pin, Bit_RESET);
}
...
{
    GPIO_WriteBit(bus->gpiox, bus->scl_gpio_pin, Bit_RESET);
}
